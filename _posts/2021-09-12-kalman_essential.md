...
excerpt: "칼만필터를 이해하고 유도한 후, 사용해보자"
...
PAPER: [discrete 상태 추정, DEKF](/assets/ICVES.2018.8519509.pdf)

## 1. Introduction
칼만필터는 잘 알려진 추정기(Estimator)이자, 확률 필터(Filter)이다. 본 글을 통해 선형 칼만필터를 정리해보자.
</br>
...
위 그림은 유도된 칼만필터의 전체식을 보여주고 있다. Linear System으로 표현된 상태 공간에서의 수식유도라 바로 위그림의 확률 수식과 달라보이지만, 정확히 같은 수식이다. [PAPER Reference](/assets/ICVES.2018.8519509.pdf)로 제공한 링크를 통해 차량 상태 추정기들을 살펴보면 위의 수식에 따라 유도되어 사용하고 있음을 알 수 있다. 해당 논문은 Extended Kalman Filter이나, 같은 수식으로 이해해도 무방하다.
...
$$
J_m={1/2}(y_k−h({x_{k}}))^{T}R^{-1}(y_k−h({x_{k}}))+{1/2}(x_k−f({x_{k−1})})^{T}P^{−1}_{k|k−1}(x_k−f({x_{k−1}}))
$$
</br>
...
$$P(x|z) = P(z|x) * P(x) / P(z)$$
최종적으로 출력하게 될 `state`의 확률은 측정값과 모델 예측값의 중간 어디쯤에 존재하고 있으니, 두 값의 중간 분산값을 기준으로 하여 Normalized하여 계산한 값 (다시 말해, weighted mean)이 P(x|z)이다. 
...
1) 출력 State
$X_{new} = \frac{(X_{predict}*1/Q + X_{measure}*1/R)}{(1/Q + 1/R)}$ 
$X_{new} = \frac{(X_{predict}*R + X_{measure}*Q)}{(Q+R)}$
...
- 측정값과 모델이 Correlation 관계에 있는 칼만 시스템 분산 이동 과정
X는 종방향 위치, $\dot X$는 종방향 속도이고, 측정값은 종방향 위치(X)인 아래 예시를 살펴보자.
> 모델 eq.
...
* `Eigen` 라이브러리를 이용하여 칼만필터 구현 (중)


## 4. Conclusion
어떤가, 재밌지 않은가?  
Extended Kalman도 비선형 모델을 선형화해서 선형 칼만과 똑같이 유도해서 푸는 방식이다. 어차피 dt 로 쪼개서 살펴보는 것이니까 군소 선형화해도 결과값은 좋게 나올 수 있다.
