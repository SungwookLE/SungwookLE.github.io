...
## 01. Introduction
### 1-1. RNN
- 시계열 데이터가 주어졌을 때, 이에 대한 특징을 학습하고 다음 시점의 출력을 예측할 수 있는 뉴럴네트워크 모델임
    - 연속적인 데이터 (자연어, 어떠한 출력 신호, 다이나믹 시스템)
- Basic RNN의 경우 성능이 좋지 않다는 문제가 있었음. 이를 살펴봄으로써 왜? 트랜스포머가 등장하게 되었는지를 알아보자.
- RNN(Recurrent Neural Network)의 동작 방식
    - ![](img/2024-08-24-20-10-22.png)
    - $h_t = tanh(x_tW_x + h_{t-1}W+b)$
        - $h_1 = tanh(x_1W_x + b)$
        - $h_2 = tanh(x_2W_x + h_1W_h+b)$
        - $\hat{y_3} = h_3W_y + b_y$
...
- 이렇게 함으로써, `h`라는 히든 스테이트가 이전 시퀀스 입력에 대한 특징을 표현하게 하려고 하였다. (이전 정보를 담아내게 하려고 하였음)
    - 이전 스텝의 `h`를 입력해줌으로써, 다음 스텝의 예측을 하게 하려고 했던 것

- 이러한 구조적 특징은, 가변적인 입력도 처리할 수 있는 장점이 있었음
    - 즉, 자연어 예측이라고 생각해보면 문장의 길이가 달라져도, RNN의 파라미터는 $W_x, b, W_h$는 공용 파라미터로 사용되는 것이니, 학습도 수월했고, 이를 통한 인퍼런싱도 수월했다.
    - 만약, RNN이 아닌 MLP라면, 어떻게 시계열 데이터를 학습하고 표현할 수 있었을까?
        - 문장 토큰마다 각각의 MLP를 배치하는 형태가 되어야 했을 것이고, 토큰의 길이를 통일시켜주어야 했을 것

- 허나, RNN도 단점이 있었으니(구조에서 발생하는 한계), `Vanishing gradient` 문제가 발생한다.
    - 멀수록 잊혀지는 문제 (학습단계에서의 `back propagation`)
        - activation 함수의 미분값이 -1~1사이의 값이고, 이게 누적(곱)되기 때문임
    - 갈수록 흐려지는 문제 (추론단계에서의 `forward propagation`) 
        - activation 함수의 출력값이 -1~1사이의 값이고, 이게 누적(곱)되기 때문임
    - 왜 그럴까?
        - RNN은 공통된 weight 파라미터를 가지고 구성된 모델이기 때문에, 문장의 길이가 길어서, 단어 토큰 사이의 거리가 멀다면, 역전파되는 `Loss`의 크기도 작을 것이고, 반대로 추론 단계에서는 반영되는 정도가 옅어짐
        - 왜냐면, activation function을 생각해보면 보통 `tanh` -1~1사이의 값을 갖는 비선형 함수를 사용하게 되는데, 문제는 입력값이 끝단에 있을 떄는 기울기가 0인 함수이기 때문임
            - 곱셈의 중첩때문에 값이 점점 작아진다!
    - 이를 수식으로 표현한다면, 아래와 같음
        - $L_{total} = L_1 + L_2 + L_3 + L_4 + ...$
            - $\frac{\partial L_4}{\partial W} = weight_1 X_1 + weight_2 X_2 + weight_3 X_3 + weight_4 X_4 + ... $
            - $weight_1 << weight_2 << weight_3 << weight_4 $ 이기 때문임

- RNN의 사용에 따른 유형 구분
    1. One to Many: image captioning이 대표적임
    2. Many to One: 문장 Score, 감정 점수 등
    3. Many to Many: 번역 (`seq2seq`) 등

### 1-2. Seq2Seq
- Many to Many 문제를 풀기위한 방법으로 제시되었던 `Seq2Seq` 모델에 대해서 살펴보고, 이 모델의 한계를 살펴보도록 하자.
- Seq2Seq: 인코더(RNN)+디코더(RNN)을 붙여서 Seq입력에 대한 Seq출력을 수행하는 네트워크였음
    - 인코더의 마지막 h(called `context vector`)를 디코더의 처음 h로 사용하였음
    - ![](img/2024-08-24-20-53-09.png)
...
- Seq2Seq의 문제점
    - 멀수록 잊혀지는 문제(인코더, 디코더 둘다 해당함)
    - `Context Vector`에는 마지막 단어의 정보가 가장 뚜렷하게 담기게 되고, 이러한 Context를 가지고 Decode되니, 마지막 단어만을 가장 열심히 보게되는 구조임 
        - 인간의 사고방식도 아님
