...
- Generative Model은 기본적으로 아래와 같이 구성됨.
    - 오토인코더의 경우에 디코더 부분을 generative model로 사용할 수 있었음을 기억하자
![](./img/2022-02-06-17-08-18.png)
- GAN(Generative Adversarial Networks)은, 일반적인 generative model을 통해 나온 결과물의 성능이 떨어지니, 이를 보완하기 위해 등장한 것으로, 보완하기 위한 방법으로 `Discriminator`를 추가하였다.
![](./img/2022-02-06-17-12-20.png)
- `Discriminator`의 목적함수를 살펴보면 real data가 들어오면 커지게끔, fake data가 들어오면 작아지게끔 설계하여서 전체 목적함수가 커지면 커질 수록 fake와 real을 잘 구분하는 네트워크가 됨을 알 수 있다.

## 2. GANs(Generative Adversarial Networks)
- GAN 모델에서는 보통 log를 취한 `cross entropy` 목적함수를 많이 쓴다. 
![](./img/2022-02-06-17-16-10.png)
![](./img/2022-02-06-17-16-31.png)
- 위의 그림을 잘 이해하는 것이 중요하다.
...
- D와 G 함수 class를 `neural network`로 파라미터라이제이션하여 문제를 구체화하고 Gradient-Descent로 해를 찾는다.
- `min/max` 문제는 게임이론과 같이 하는 최대화, 하나는 최소화하여야 하기 때문에 목적함수는 `convex+concave` 형태이다.
![](./img/2022-02-06-17-29-32.png)
    - `min/max` 문제를 푸는 것이고, saddle point가 최적 해이기 때문에 `training instability`가 존재하는 것이 고질적인 문제이다.
    - `training instability`를 조금이나마 해소하고 `discriminator`와 `generator`의 학습을 조금 더 잘 되게 하기 위해 **Alternating Gradient Descent**라는 것을 이용한다.
    - **Alternating Gradient Descent**는 `generator`를 한번 학습할 때, `discriminator`는 **k**번 반복학습하는 것으로 *highly nonlinear*할 때 러닝레이트를 작게하고, 여러번 반복시킴으로써 학습을 조금 더 잘되게 하는 테크닉이다.
...
### 2-1. 참고(fake data generator)
- data `generator`로 사용할 수 있는 것 중 배운 것들을 소개해보자
    - `autoencoder` 학습 후 `decoder` 사용: 이 때, internel feature는 가우시안 distribution 의 랜덤 인풋, 
...
- Autoencoder로 결측데이터에 대한 `matrix completion`을 수행하였는 데, 결과가 조금 아쉬울 경우 `RBMs`를 구려해보면 성능이 향상될 수 도 있다.
- 본 강의에서는 binary random data를 예시로 하여 <1,0>이 나오는 데이터에서 확률만 주어지면 데이터를 생성해낼 수 있다는 예시를 베이스로 해서 시작하겠다. (이를 `깁스 샘플링, Gibs Sampling`이라고 한다 함)
