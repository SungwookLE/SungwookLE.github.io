...
7. 멀티프로세스에 대한 직관적인 해석들 (p98)

8. concurrent SW 실행 원리 (멀티 프로세스, 멀티 쓰레드)
- 실행 흐름 차이 (맨 오른쪽이 멀티 프로세스-멀티 테스크)
    - ![](img/2024-09-04-09-19-02.png)

- SW에서 하드웨어에 접근하려면, 반드시 시스템콜(API)를 이용해야함   
    - 왜냐면, 모든 SW 것들을 커널에서 관리하고자 하기 위함

9. 원래는 유저가 직접 어떤 wait 상태에 빠지면, wait할 게 아니라, 다른 프로세스가 돌아가게끔 코딩해주어야했는데, 리눅스와 같이 커널이 존재한다면, 커널이 이걸 관리해주어서, wait 들어가면 다른 프로세스를 실행시키게되니까.., MCU 에서의 멀티 프로세스 코드와 다르게 조금 더 수월하지
다만 커널은 연결리스트로 정보들을 관리하기 떄문에 조금 느릴 수 있음.
    - 이걸 커스터마이징에서 임베디드 리눅스 시스템에서 커널을 손보는 방식으로 성능을 더 빠르게 할 수 도 있음


10. 멀티 프로세스를 위한 (부모-자식)
- `fork()`를 통해 자식 프로세스를 생성할 수 있음
    - `fork()` 시스템콜을 통한 프로세스 생성
    - 포크를 한다는 것은 메모리를 신규 프로세스를 위해 할당하는 것을 의미함
    - 포크는 자식 프로세스의 ID를 반환함
- 부모의 전역변수를 자식 프로세스가 `capture`의 과정으로 물려받게 됨

- 부모 프로세스와 자식 프로세스는 concurrent하게 코드 진행됨 (CPU코어가 1개라면, 균등분할되어서..!)

- `exec()`는 자식 프로세스를 별도의 독립된 프로그램으로서 실행시킬 때 사용하는 함수임
- ![](img/2024-09-05-09-32-04.png)
```c
if (pid=fork() ==0){
    exec(arguemnt);
    exit(1); // 여기까지 라인이 내려오면 에러임
}
```

11. 멀티 프로세스를 동시에 실행시키는 원리
    1. 사용자 프로세스와 커널 프로세스
...
12. 멀티프로세스를 위한 가상 메모리
- 멀티 프로세스가 실현되기 위해서는 가상메모리 개념이 지원되어야 한다.
    - 그러지 않으면, 다른 프로세스가 다른 프로세스의 영역을 침범하게 되겠지.
- 모든 프로그램은 가상메모리 영역을 대상으로 컴파일/링킹되어 파일에 저장한 상태임
- 따라서 cpu가 이 프로그램을 읽어서 디코딩해보아도 접근할 메모리 영역도 여전히 원래 가상메모리 영역이다.
- 하지만, 이 프로그램을 실제 메모리(DRAM)에 동적 배치되었으므로 프로그램이 물리 메모리에 배치된 위치를 별도의 테이블에 기록해두고 이 테이블을 이용해서 가상 메모리 접근시 가상 주소를 실제 물리 메모리 주소로 변환해주면된다.
- 가상메모리의 효과
    - 모든 프로그램은 각자 가상메모리 영역을 모두(32비트 주소 영역이면, 0~4G전체)을 모두 사용할 수 있고, 컴파일/링킹되어 프로그램 이미지가 생성됨
    - 프로그램이 실행되어 DRAM에 동적 배치되어 프로세스가 생성될 때 각 프로세스마다 주소영역 변환 테이블(페이지 테이블)을 생성/업데이트 한다.
    - 페이지 크기를 잘게 쪼개면, 아래 오랜쪽 처럼 메모리의 작은 가용영역도 재사용할 수 있다.
- 온디멘트 페이징
    - 요구가 있으면 페이징하는것(가상메모리->물리RAM 메모리)
- 스왑
    - 물리 메모리가 부족해도, 프로세스가 동작하는 이유는 스왑을 통해 저장장치(예를들면 하드디스크)의 메모리를 사용하는 것 -> 많이 느려서 뻑난것처럼 멈추는 현상

13. 공유 메모리를 통한 프로세스간 통신
...
15. 프로그램 생성과 동시에 생기는 표준 입출력
- `stdin, stdout, stderr`

16. 파일에 접근하기 위해 최초로 파일을 열어서, 파일 구조체로 포인터를 얻기 위한 표준 함수는
- `fopen()`

17. 하드웨어 장치에 접근하기 위한 저수준의 함수이며, 커널이 제공함
- system call

18. 프로그램 실행시, 점프할 함수를 내 프로그램에 동적으로 결합
- 동적 링킹
- 컴파일 후 생기는 오브젝트파일(리눅스: `*.so`, 윈도우즈: `*.dll`)을 링크시킴

19. 리눅스에서 다룰 수 있는 모든 장치를 열기 위한 함수로, 파일 descriptor를 반환하는 함수
- `open()`

20. 하나의 파일의 특정 영역을 복수의 프로세스가 접근할 때 I/O 잠금해주는 함수
- `flock()`

21. 운영체제와 나 사이에 인터페이싱해주는 프로세스로, 프로그램을 커널 위에 올려주는 역할, (예를들어 `fork(), exec()`) 
- 쉘. shell

22. 파일의 소유자는 아니지만, 실행할 때 잠시 소유자의 권한을 획득
- `set user id`: effective id

23. `fork`를 이용해서 자식을 생성하는 과정
- 자식 프로세스는 부모 프로세스를 완전히 복제
- 부모 프로세스의 fork() 리턴값은 자식의 pid임
- 부모 프로세스는, 자식 프로세스가 끝날 때까지 기다리지 않고, 바로 실행을 이어감

24. 하드웨어 자원의 특정 영역을 메모리의 한 영역으로 매핑해서, 마지 메모리에 읽고 쓰는 방식으로 자원에 접근하게 해주는 함수
- `mmap`
